// using g++ -o hospitalC -std=c++11 hospitalC.cpp
# include <iostream>
# include <sstream>
# include <iomanip>
# include <list>
...
int main(int argc, char* argv[])
{
    ...
    /**********************************       boot-up       ***************************************/
    cout << "Hospital C is up and running using UDP on port <" << PORT_C << ">​." << endl;
    ...
    cout << "Hospital C has total capacity ​<" << argv[2] << ">​ and initial occupancy ​<" << argv[3] << ">​." << endl;
    ...
    int sockHosC = socket(AF_INET, SOCK_DGRAM, 0);
    if (sockHosC == -1)
    {
        cerr << "Hospital C can't create UDP socket! Quitting..." << endl;
        return -1;
    }
    // bind the socket to an IP / port for future communication with scheduler
    sockaddr_in hint;  // ipv4 add
    hint.sin_family = AF_INET;
    hint.sin_port = htons(PORT_C);  // host to network short, little -> big endian
    inet_pton(AF_INET, LOCAL_HOST, &hint.sin_addr);  // pass in the buffer(pointer)
    if (::bind(sockHosC, (struct sockaddr *) &hint, sizeof(hint)) == -1)  // change the scope
    {
        cerr << "Hospital C can't bind UDP socket to IP/port! Quitting..." << endl;
        return -2;
    }
    ...
    if (availability < 0 || availability > 1)
    {
        ...
        cout << "Hospital C has capacity = ​<" << argv[2] << ">, occupation = <" << argv[3] << ">​, availability = <None>" << endl;
    }
    else
    {
        cout << "Hospital C has capacity = ​<" << argv[2] << ">, occupation = <" << argv[3] << ">​, availability = <" << availability << ">" << endl;
    }
    string a = to_string(availability);
    string port = to_string(PORT_C);
    string a_str = a + " " + port + " " + argv[2] + " " + argv[3];  // capacity: argv[2], occupancy: argv[3]
    
    // send initial state to scheduler
    int hosSend = sendto(sockHosC, a_str.c_str(), a_str.size()+1, 0, 
                    (const sockaddr *)&scheduler, scheduler_len);
    if (hosSend == -1)
    {
        cout << "Hospital C failed to send information! Quitting..." << endl;
        return -2;
    }
    ...
    do
    {
        string buffer_sch = UDP_recvfrom_scheduler(scheduler, sockHosC);
        cout << "Hospital C has received input from client at location ​<" << buffer_sch << ">." << endl;
        ...
        if (!map.contains_node(home))
        {
            ...
            dis_None = true;
            d = NULL;
            cout << "Hospital C does not have the location ​<" << home << ">​ in map" << endl;
            string fail = string("None") + " " + string("None") + " " + argv[1];
            int hosSend_2 = sendto(sockHosC, fail.c_str(), fail.size()+1, 0, 
                    (const sockaddr *)&scheduler, sizeof(scheduler));
            if (hosSend_2 == -1)
            {
                cout << "Hospital C failed to send \"location not found\" to the Scheduler! Quitting..." << endl;
                return -2;
            }
        ...
        cout << "Hospital C has found the shortest path to client, distance = <" << c1 << ">" << endl;
        cout << "Hospital C has the score = ​<" << score << ">" << endl;

        string c2 = argv[1];  // index
        string c = score + " " + c1 + " " + c2;  // score, distance, index
        hosSend_2 = sendto(sockHosC, c.c_str(), c.size()+1, 0, 
                    (const sockaddr *)&scheduler, sizeof(scheduler));
        if (hosSend_2 == -1)
        {
            cout << "Hospital C failed to send score! Quitting..." << endl;
            return -2;
        }
        cout << "Hospital C has sent score = ​<" << score << ">​ and distance = ​<" << c1 << ">​ to the Scheduler" << endl;

        /*************************      recv the scheduler's response      ***************************/
        string buffer_end = UDP_recvfrom_scheduler(scheduler, sockHosC);
        ...
        if (buffer_end == "1")
        {
            ...
            if (hosSend == -1)
            {
                cout << "Hospital C failed to send updated information! Quitting..." << endl;
                return -2;
            }
        }
    } while(true);
